...
| Architecture comparison | quality gate target만으로 strict coverage처럼 보일 수 있었음 | `compare_architectures.py --audit-dir`가 `strict_result_audit.csv`의 `audit_pass=true` row만 publishable best로 사용. postprocess wrapper는 audit evidence를 compare 단계에 자동 전달 |
