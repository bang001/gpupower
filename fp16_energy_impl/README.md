...
`compare_architectures.py`도 기본적으로 `target_pass=true`이고 `measurement_grade=strict_nvml_counter`인 thread point만 `architecture_best_fp16.csv`와 best summary figure에 사용한다. `quality_pass=true`지만 utilization saturation target이 아니거나 power-trace fallback인 row는 diagnostic으로 남기되, 최종 best pJ/bit 그림에는 올리지 않는다. Diagnostic 후보까지 강제로 best table에 넣어야 할 때만 `--allow-diagnostic-best`를 사용한다. quality gate metadata가 없는 legacy result directory는 기본적으로 `missing_quality_gate_no_best`로 reject되며, old result를 그림에 넣어야 할 때만 `--allow-legacy-best`를 명시한다.
