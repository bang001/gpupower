#!/usr/bin/env bash
# Read-only SM/L2/DRAM energy decomposition launcher.
set -euo pipefail
...
if [[ -z "$PY" ]]; then
    ...
    echo "      sudo env PY=/path/to/venv/bin/python ./run_energy_decomp.sh ..." >&2
    exit 1
fi
...
exec "$PY" "$SCRIPT_DIR/dram_energy_decomp_cupy.py" \
  --targets 50 75 100 \
  --repeats 35 \
  --warmup-repeats 5 \
  --phase-seconds 15 \
